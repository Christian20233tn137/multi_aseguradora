...
const ConfigurarCuotas = () => {
  ...
  const [emisiones, setEmisiones] = useState(null); // Cambiado a null para indicar carga inicial
  ...
  useEffect(() => {
    const fetchEmisionesActuales = async () => {
      try {
        ...
        if (data.length > 0) {
          setEmisiones(data[0].cuotaMensual); // Accede al primer elemento del arreglo
        } else {
          throw new Error("No se encontraron datos de emisiones");
        }
      } catch (error) {
        ...
        if (result.isConfirmed) {
          try {
            ...
            const response = await axios.put(
              `https://lz2k718xka.execute-api.us-east-1.amazonaws.com/nar/cuotas/id/67f7672cd291e7c96c2ebef3`,
              {
        ...
        <>
          ...
          <div className="flex items-center justify-center mt-10">
            <button
              ...
              disabled={loading}
              className={`w-30 py-3 px-6 text-lg font-semibold text-white rounded-lg ${
                loading ? "bg-gray-400" : "botones hover:bg-blue-700"
              }`}
            >
