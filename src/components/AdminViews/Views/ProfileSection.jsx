...
const ProfileSection = () => {
  ...
  const fetchAllStats = async (mesSeleccionado) => {
    ...
    try {
      ...
      console.log("Agentes Activos:", agentesRes.data);
      console.log("Cotizaciones:", cotizacionesRes.data);
      console.log("Emisiones:", emisionesRes.data);
    ...
    <div className="flex flex-col p-6">
      ...
      <div className="flex flex-wrap justify-center gap-6">
        ...
        <div className="flex flex-col items-center flex-1 p-12 bg-blue-100 rounded shadow min-w-[300px]">
          ...
          <p className="text-4xl font-bold">{data.agentesActivos}</p>
        </div>

        {/* Cotizaciones */}
        <div className="flex flex-col items-center flex-1 p-12 bg-blue-100 rounded shadow min-w-[300px]">
          <h2 className="text-xl font-bold mb-2">Cotizaciones</h2>
          <p className="text-4xl font-bold">{data.cotizaciones}</p>
        </div>

        {/* Emisiones */}
        <div className="flex flex-col items-center flex-1 p-12 bg-blue-100 rounded shadow min-w-[300px]">
          <h2 className="text-xl font-bold mb-2">Emisiones</h2>
          <p className="text-4xl font-bold">{data.emisiones}</p>
        </div>
      </div>
    </div>
  );
